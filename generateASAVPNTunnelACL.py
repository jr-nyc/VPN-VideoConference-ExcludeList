    ...
    for x in ipList:
        ...
        if ipAddr != "error":
            ...
            if str(ipMask) == "255.255.255.255":
                ...
                updateList.append(f"access-list ASAVPNTunnelACL standard permit host {ipAddr}")
            else:
                updateList.append(f"access-list ASAVPNTunnelACL standard permit {ipAddr} {ipMask}")
...
def genACLFile (filename,*ipLists):
    ...
    with open(filename, 'w+') as outfile:
        yaml.dump(ymlData, outfile, default_flow_style=False,
                  explicit_start=True)
    print(filename)
...
def getMSTeamsIPs ():
    ...
    html = urlopen(
        "https://endpoints.office.com/endpoints/worldwide?clientrequestid={}".format(uuid.uuid4()))
...
if __name__ == '__main__':
    ...
    genACLFile("roles/ASAVPNTunnelACL/var/main.yml",
               staticACL, zoomACL, webexACL, MSTeamsACL)
